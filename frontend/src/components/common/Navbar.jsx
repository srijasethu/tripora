        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#C85A32] to-[#8C3414] p-0.5 shadow-md shadow-[#C85A32]/20 group-hover:scale-105 transition-transform duration-300">
              <div className={`w-full h-full rounded-[10px] flex items-center justify-center ${scrolled ? 'bg-[#FAF6F0]' : 'bg-[#141518]'}`}>
                <Compass className="w-4 h-4 text-[#C85A32] group-hover:rotate-45 transition-transform duration-500" />
              </div>
            </div>
            <div className="flex flex-col">
              <span className={`font-cinzel text-xl font-bold tracking-widest leading-tight ${scrolled ? 'text-[#141518]' : 'text-white'}`}>
                TRIPORA
              </span>
          ...
          {/* Desktop Nav Links */}
          <nav className={`hidden md:flex items-center gap-8 px-6 py-2 rounded-full border backdrop-blur-md transition-colors ${
            scrolled ? 'bg-black/5 border-black/10 text-[#141518]' : 'bg-white/10 border-white/20 text-white'
          }`}>
            <button
            ...
            <button
              onClick={() => scrollToSection('detour')}
            ...
            <button
              onClick={() => scrollToSection('reality-engine')}
              className="text-sm font-medium hover:text-[#C85A32] transition-colors cursor-pointer"
            >
              Reality Engine
            </button>
          ...
          <div className="hidden md:flex items-center gap-4">
            <button
              onClick={() => alert('Tripora Auth — Sign In coming soon!')}
              className={`text-sm px-3 py-2 font-medium transition-colors cursor-pointer ${
                scrolled ? 'text-[#141518]/80 hover:text-[#141518]' : 'text-white/90 hover:text-white'
              }`}
            >
              Sign In
            </button>
